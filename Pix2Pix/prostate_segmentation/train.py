...
import dmnet_gluon_init as net_init
from utils import *
import numpy as np
import argparse
import os
...
class Training(InitParams):
    ...
    def set_iter(self):
        ...
        self.train_iter = io.NDArrayIter(self.im[self.train_idx[:self.train_amount], :, :, :, :],
                                         self.lab[self.train_idx[:self.train_amount], :, :, :],
                                         batch_size=self.train_batch_size, shuffle=True)
        self.val_iter = io.NDArrayIter(self.im[self.val_idx[:self.val_amount], :, :, :, :],
                                       self.lab[self.val_idx[:self.val_amount], :, :, :],
    ...
    def run(self, lg):
        ...
        for epoch in range(self.epochs):
            ...
            # VALIDATION
            if self.val_interval & (epoch + 1) % self.val_interval == 0:
...
class Model(Training):
    ...
    def init_params(self):
        ...
        self.net.collect_params().initialize(initializer.Xavier(), ctx=self.ctx)
        print(self.ctx)
    ...
    def save_params(self, epoch, current_dice, best_dice):
        ...
        if current_dice > best_dice:
            ...
            self.net.save_params("{:s}_best.params".format(self.dir_out + self.prefix))
        if self.save_interval and ((epoch+1) % self.save_interval == 0):
            self.net.save_params("{:s}_{:04d}_{:.2f}.params".format(self.dir_out + self.prefix, epoch, current_dice))
        return best_dice
